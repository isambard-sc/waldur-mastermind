...
@shared_task(name="waldur_mastermind.marketplace_openportal_remote.sync_offering_users")
def sync_offering_users():
    logger.info("Syncing offering users for OpenPortal Remote marketplace plugin.")
...
@shared_task(
    name="waldur_mastermind.marketplace_openportal_remote.mark_offering_backend_as_disconnected_after_timeout"
)
def mark_offering_backend_as_disconnected_after_timeout():
    logger.info(
        "Marking OpenPortal Remote offering backend as disconnected after timeout."
    )
...
@shared_task(name="waldur_mastermind.marketplace_openportal_remote.sync_resources")
def sync_resources():
    """
    ...
    """
    logger.info("Syncing resources for OpenPortal Remote marketplace plugin.")
...
@shared_task(
    name="waldur_mastermind.marketplace_openportal_remote.send_messages_about_pending_orders"
)
def send_messages_about_pending_orders():
    """Send a message about pending orders created 1 hour ago to MQTT"""
    logger.info(
        "Sending messages about pending orders for OpenPortal Remote marketplace plugin."
    )
    offering_ids = get_offering_ids_for_active_subscriptions(

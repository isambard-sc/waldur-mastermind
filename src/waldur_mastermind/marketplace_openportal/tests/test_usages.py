...
from waldur_openportal.models import Allocation, AllocationUserUsage
from waldur_openportal.parser import OpenPortalReportLine
from waldur_openportal.tests import factories as openportal_factories


class ComponentUsageTest(test.APITransactionTestCase):
    def setUp(self):
        fixture = structure_fixtures.ProjectFixture()
        service_settings = structure_factories.ServiceSettingsFactory(type="OpenPortal")
        offering = marketplace_factories.OfferingFactory(
            type=PLUGIN_NAME, scope=service_settings
        )
        plan = marketplace_factories.PlanFactory(offering=offering)
        self.allocation = openportal_factories.AllocationFactory()
